...
**If using WSL, always build the project in a directory on the Linux partition (somewhere under WSL `cd ~`). You will have to copy your project from the Windows partition to the Linux partition. NEVER build the project in a directory on the Windows partition (`/mnt/c/...`), as this may give unpredicatable results.**
