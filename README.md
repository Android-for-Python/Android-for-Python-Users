...
**If using WSL, always build in a directory on the Linux partition (somewhere under `cd ~`). You will have to copy your project from the Windows partition to the Linux partition. NEVER build with the project directory on the Windows partition (`/mnt/c/...`), this may give unpredicatable results.**
