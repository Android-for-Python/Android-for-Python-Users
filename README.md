...
Without special handling Kivy and asyncio would block one another on a single task OS like Android or iOS, and have OS dependent behavior on a multitasking OS. So we must start a Kivy app as an asyncio coroutine. We normally start a Kivy app with:

```
ExampleApp().run()
```

The Kivy App class defines a coroutine `async_run()` which starts Kivy. Instead of the above we start the Kivy app with:
